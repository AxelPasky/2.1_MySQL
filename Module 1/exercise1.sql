...
SELECT nombre, CAST(precio AS INT) as precio_int  FROM producto; 
...
SELECT producto.nombre AS nombre_producto , producto.precio , fabricante.codigo , fabricante.nombre AS nombre_fabricante FROM producto RIGHT JOIN fabricante ON producto.codigo_fabricante = fabricante.codigo;
...
SELECT producto.precio,producto.nombre, fabricante.nombre AS fabricante FROM producto INNER JOIN fabricante ON fabricante.codigo=producto.codigo_fabricante WHERE producto.precio >= 180 ORDER BY 	producto.precio DESC , producto.nombre ASC;
...
--3 Retorna el llistat de professors/es que no han donat d'alta el seu número de telèfon en la base de dades i a més el seu NIF acaba en K.
SELECT nif ,tipo, telefono FROM persona WHERE tipo = "profesor" AND nif LIKE "%K" AND telefono IS NULL;

--4 Retorna el llistat de les assignatures que s'imparteixen en el primer quadrimestre, en el tercer curs del grau que té l'identificador 7.
SELECT nombre , cuatrimestre , curso , id_grado FROM asignatura WHERE cuatrimestre = 1 AND curso = 3 AND id_grado = 7;

--5 Retorna un llistat dels professors/es juntament amb el nom del departament al qual estan vinculats/des. El llistat ha de retornar quatre columnes, primer cognom, segon cognom, nom i nom del departament. El resultat estarà ordenat alfabèticament de menor a major pels cognoms i el nom.
SELECT persona.apellido1, persona.apellido2, persona.nombre , departamento.nombre AS nombre_departamento FROM persona INNER JOIN profesor ON profesor.id_profesor = persona.id INNER JOIN departamento ON profesor.id_departamento = departamento.id ORDER BY persona.apellido1,persona.apellido2,persona.nombre ASC;

--6 Retorna un llistat amb el nom de les assignatures, any d'inici i any de fi del curs escolar de l'alumne/a amb NIF 26902806M.
SELECT asign.nombre , curs.anyo_inicio , curs.anyo_fin , p.nif FROM curso_escolar AS curs INNER JOIN persona AS p ON p.id = curs.id INNER JOIN asignatura AS asign ON asign.id = curs.id WHERE p.nif = '26902806M';

--7 Retorna un llistat amb el nom de tots els departaments que tenen professors/es que imparteixen alguna assignatura en el Grau en Enginyeria Informàtica (Pla 2015).
...
--8 Retorna un llistat amb tots els/les alumnes que s'han matriculat en alguna assignatura durant el curs escolar 2018/2019.
...
SELECT dep.nombre,persona.apellido1,persona.apellido2,persona.nombre FROM persona RIGHT JOIN profesor AS prof ON prof.id_profesor = persona.idRIGHT JOIN departamento AS dep ON dep.id = prof.id_departamento WHERE persona.tipo = "profesor" ;
...
SELECT COUNT (YEAR(fecha_nacimiento)) as alumnos_1999 FROM persona WHERE tipo = 'alumno' AND YEAR(fecha_nacimiento)= 1999;
